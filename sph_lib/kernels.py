...
class Kernel:
	...
	def evaluate_gradient(self, r_ij_vec, h=None, H=None):
		...
		if h is not None:
			...
			norm = self._kernel_sigma() / h**self.dim         # (N, 1)
		...
		# ======================
			...
			det_H = np.linalg.det(H)                           # (N,)
			norm = self._kernel_sigma() / det_H[:, None]       # (N, 1)
			...
			# dK/dq
			dK_dq = self._kernel_gradient_values(q) # np.where(mask, -2 * q * np.exp(-q**2), 0.0)

			# ∇q = H⁻ᵀ ξ / q
			grad_q = np.einsum('nij,nmj->nmi', H_inv_T, xi) / (q[..., None] + 1e-12)                         # (N, M, d)

			# ∇W = (1/detH) dK/dq ∇q
			grad_W = norm[..., None] * dK_dq[..., None] * grad_q
			return grad_W
...
class gaussian():
	...
	def evaluate_gradient(self, r_ij_vec, h=None, H=None):
		...
		if h is not None:
			...
			mask = q <= 3
			dW_dq = np.where(mask, -2 * q * np.exp(-q**2), 0.0)
		...
		# ======================
			...
			norm = self.sigma() / det_H[:, None]               # (N, 1)

			mask = q <= 3
			dK_dq = np.where(mask, -2 * q * np.exp(-q**2), 0.0)
