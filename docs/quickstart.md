...
`smudgy` is a python package with a C++ and optional OpenMP backend for point cloud smoothing, interpolation and grid deposition -- lightning fast, scalable and memory-efficient.
...
field_values    = np.random.uniform(0, boxsize, (N, num_fields))
weights         = np.ones(N)
